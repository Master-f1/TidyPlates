...
local helppanel = PanelHelpers:CreatePanelFrame( "TidyPlatesInterfaceOptionsHelp", "Помощь" )
...
local function ActivateInterfacePanel()
	...
	panel.label:SetText(
		"Пожалуйста, выберите тему для ваших основных и вторичных специализаций; "..
		"Соответствующая тема будет автоматически активирована при включении специализации.")
	...
	panel.PrimaryLabel:SetText("Набор талантов 1:")
	...
	-- [[ Autohide:
	panel.AutoHide = PanelHelpers:CreateCheckButton("TidyPlatesOptions_Autohide", panel, "Показать/скрыть полосы, когда бой начинается и заканчивается")
...
local function ActivateHelpPanel()
	...
	helppanel.vkeylabel:SetText("Показать полосы:|cFFFFFFFF Вы также можете переключать полосы, используя:"..
								" '|rV|cFFFFFFFF' для врагов, '|rShift-V|cFFFFFFFF' для друзей, и '|rControl-V|cFFFFFFFF' оба.")
	...
	helppanel.vkey:SetText("Показать")
	...
	helppanel.resetreloadlabel:SetText("Сброс настроек:|cFFFFFFFF Эта кнопка сбросит переменные Tidy Plates и перезагрузит UI.  "..
										"Не сбрасывайте во время боя! ")
	...
	helppanel.resetreload:SetText("Сброс")
	...
	helppanel.notes:SetText("Если возникнут проблемы, посетите нашу страницу Wowinterface.com ")
...
TidyPlatesSlashCommands.reset = function() print("Tidy Plates: Настройки сброшены"); TidyPlatesOptions = copytable(TidyPlatesOptionsDefaults); LoadTheme(TidyPlatesOptions[activespec]) end
