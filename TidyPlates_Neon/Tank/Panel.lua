...
local TextModes = { { text = "Нет", notCheckable = 1 },
					...
					{ text = "Уровень", notCheckable = 1 } ,
					}
		
local RangeModes = { { text = "9 ярдов" } , 
					{ text = "15 ярдов" } ,
					{ text = "28 ярдов" } ,
					{ text = "40 ярдов" } ,
					}
...
local DebuffModes = { 
					{ text = "Показать всё", notCheckable = 1 } ,
					{ text = "Фильтр", notCheckable = 1 } , 
...
local function CreateInterfacePanel( Name, Title, Parent, VariableTarget )  
	...
	panel.ApplyButton:SetText("Применить")
	...
	panel.OpacityNonTarget = PanelHelpers:CreateSliderFrame(Name.."_OpacityNonTargets", Column1, "Нет цели непрозрачность:", .5, 0, 1, .1)
	...
	panel.OpacityHideNeutral = PanelHelpers:CreateCheckButton(Name.."_OpacityHideNeutral", Column1, "Скрыть нейтральных")
	...
	panel.OpacityHideNonElites = PanelHelpers:CreateCheckButton(Name.."_OpacityHideNonElites", Column1, "Скрыть не элитных")
	...
	-------------------
	-- General Scale
	panel.ScaleGeneral = PanelHelpers:CreateSliderFrame(Name.."_ScaleGeneral", Column1, "Общий масштаб:", 1, .5, 2, .1)
	panel.ScaleGeneral:SetPoint("LEFT")
	panel.ScaleGeneral:SetPoint("TOP", panel.OpacityHideNonElites, "BOTTOM", 0,-32)
	-- Loose Units Scale
	panel.ScaleDanger = PanelHelpers:CreateSliderFrame(Name.."_ScaleLoose", Column1, "Масштаб при потере угрозы:", 1.5, 1, 2, .1)
	...
	-- Hide Non-Elites Checkbox
	panel.ScaleIgnoreNonElite = PanelHelpers:CreateCheckButton(Name.."_ScaleIgnoreNonElite", Column1, "Игнорировать не элитных")
...
-- [[
	...
	panel.AggroIndDesc:SetText("Индикатор угрозы:")
	...
	-- Health Bar
	panel.AggroHealth = PanelHelpers:CreateCheckButton(Name.."_AggroHealth", Column1, "Цвет полос здоровья")
	panel.AggroHealth:SetPoint("LEFT")
	panel.AggroHealth:SetPoint("TOP", panel.AggroIndDesc, "BOTTOM",0, -4)
	-- Border
	panel.AggroBorder = PanelHelpers:CreateCheckButton(Name.."_AggroBorder", Column1, "Свечение")
...
--]]
	...
	panel.AggroColorDesc:SetText("Цвет угрозы:")
	...
	-- Loose Units
	panel.AggroLooseColor = PanelHelpers:CreateColorBox(Name.."_AggroSafeColor", Column1, "Цвет потери", 0, .5, 1, 1)
	panel.AggroLooseColor:SetPoint("LEFT", 24)
	panel.AggroLooseColor:SetPoint("TOP", panel.AggroColorDesc, "BOTTOM", 0,-8)

	-- Tanked Units
	panel.AggroTankedColor = PanelHelpers:CreateColorBox(Name.."_AggroDangerColor", Column1, "Цвет угрозы", 0, .5, 1, 1)
	...
	-- Health Text
	-------------------
	panel.HealthText = PanelHelpers:CreateDropdownFrame(Name.."_HealthText", Column1, TextModes, 1, "Текст полос здоровья:")
	...
	panel.WidgetDesc:SetText("Индикаторы & виджеты:")
	panel.WidgetDesc:SetJustifyH("LEFT")
	panel.WidgetDesc:SetPoint("LEFT", -5)
	panel.WidgetDesc:SetPoint("TOP", 0, -15)

	-- Level
	panel.LevelText = PanelHelpers:CreateCheckButton(Name.."_WidgetLevelText", Column2, "Уровень")
	panel.LevelText:SetPoint("LEFT")
	panel.LevelText:SetPoint("TOP", panel.WidgetDesc, "BOTTOM", 0, -10)
	
	-- Class Icon
	panel.WidgetClassIcon = PanelHelpers:CreateCheckButton(Name.."_WidgetClassIcon", Column2, "Иконка класса")
	panel.WidgetClassIcon:SetPoint("LEFT")
	panel.WidgetClassIcon:SetPoint("TOP", panel.LevelText, "BOTTOM", 0, -4)
	
	-- Selection Box
	panel.WidgetSelect = PanelHelpers:CreateCheckButton(Name.."_WidgetSelect", Column2, "Выбор коробки")
	panel.WidgetSelect:SetPoint("LEFT")
	panel.WidgetSelect:SetPoint("TOP", panel.WidgetClassIcon, "BOTTOM", 0, -4)

	-- Tug-o'-Threat
	panel.WidgetTug = PanelHelpers:CreateCheckButton(Name.."_WidgetTug", Column2, "Оправдание")
	panel.WidgetTug:SetPoint("LEFT")
	panel.WidgetTug:SetPoint("TOP", panel.WidgetSelect, "BOTTOM", 0, -4)

	-- Loose Color
	panel.TugWidgetLooseColor = PanelHelpers:CreateColorBox(Name.."_TugWidgetLooseColor", Column2, "Безопасно", 0, .5, 1, 1)
	...
	-- Aggro'd Color
	panel.TugWidgetAggroColor = PanelHelpers:CreateColorBox(Name.."_TugWidgetAggroColor", Column2, "Агрро", 0, .5, 1, 1)
	...
	-- Safe 
	panel.TugWidgetSafeColor = PanelHelpers:CreateColorBox(Name.."_TugWidgetSafeColor", Column2, "Другой танк", 0, .5, 1, 1)
	...
	-- Threat Wheel
	panel.WidgetWheel = PanelHelpers:CreateCheckButton(Name.."_WidgetWheel", Column2, "Колесо угрозы")
	panel.WidgetWheel:SetPoint("LEFT")
	panel.WidgetWheel:SetPoint("TOP", panel.TugWidgetSafeColor, "BOTTOM", 0, -4)


	-- Combo Points
	panel.WidgetCombo = PanelHelpers:CreateCheckButton(Name.."_WidgetCombo", Column2, "Комбо-поинты")
	panel.WidgetCombo:SetPoint("LEFT")
	panel.WidgetCombo:SetPoint("TOP", panel.WidgetWheel, "BOTTOM", 0, -8)

	-- Group Range
	panel.WidgetRange = PanelHelpers:CreateCheckButton(Name.."_WidgetRange", Column2, "Вид наблюдателя")
	...
	-- Short Debuffs
	panel.WidgetDebuff = PanelHelpers:CreateCheckButton(Name.."_WidgetDebuff", Column2, "Дебафф трекер")
...
TankPanel = CreateInterfacePanel( "TidyPlatesNeonTank", "Танк", "Tidy Plates: Neon", TidyPlatesNeonTankVariables ) 
